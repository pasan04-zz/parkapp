      ...
      <v-col cols="12" sm="10" md="8" lg="10">
        <v-card>
          <v-tabs background-color="#0036D9" class="white--text" right>
            ...
            <v-tab class="white--text">Add Message</v-tab>

            <!-- FIRST TAB- ALLE VOERTUIGEN -->
            <v-tab-item>
              <v-select
                :items="status_list"
          ...
          <!-- SECOND TAB- MESSAGES TO EMPLOYEES --> 
            <v-tab-item>
              <!-- <v-card>

                <v-textarea
                  v-model="messagetoemployee"
                  :rules="[() => !messagetoemployee || 'This field cannot be empty']"
...
export default {
  ...
  created() {
    ...
    if (this.$store.state.currentOPCO.username !== '') {
      this.loading = false;
    } else {
      this.$router.push('/');
    }
  ...
  methods: {
    ...
    checkLogin() {
      if (this.$store.state.currentOPCO.logout_time != null) {
        if (Date.now() > this.$store.state.currentOPCO.logout_time) {
          this.$store.state.currentOPCO.username = '';
          this.$store.state.currentOPCO.login_time = null;
          this.$store.state.currentOPCO.logout_time = null;
